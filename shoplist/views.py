...
from .models import Product, PaymentMethod, CreditCard, Bank, Order, OrderItem, Cart, CartItem, ShippingAddress
...
def product_list(request):
    products = Product.objects.all()
    paginator = Paginator(products, 10)  # Mostrar 10 productos por página
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    payment_form = PaymentForm()  # Formulario para capturar información de pago
    return render(request, 'product_list.html', {'products': products, 'payment_form': payment_form}, {'page_obj': page_obj})
        ...
        if form.is_valid():
            ...
            if match_token(request.user.creditcard.otp_secret, otp_code):
                ...
                try:
                    ...
                    # Guardar el pedido en el carrito de compras del usuario
...
def payment_success(request):
    if request.method == 'GET':
        ...
        order, _ = Order.objects.get_or_create(user=request.user, status='completed')
        order.payment_id = payment_id
        order.save()

        # Vaciar el carrito de compras del usuario después de una compra exitosa
        cart.items.clear()
